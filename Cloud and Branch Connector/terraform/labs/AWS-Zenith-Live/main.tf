...
module "cc-vm" {
  ...
  source             = "../modules/terraform-zenith-zscc1-aws"
...
module "cc-vm2" {
  ...
  source             = "../modules/terraform-zenith-zscc2-aws"
  cc_count           = var.cc_count
  name_prefix        = var.name_prefix
  resource_tag       = var.name_suffix
  global_tags        = local.global_tags
...
locals {
  ...
  yum update -y
  #Add hosts
  echo -e "${module.workload1.private_ip[0]} intranet.region1.acme.com" > /etc/hosts
