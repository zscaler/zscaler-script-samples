...
resource "aws_iam_policy" "cc_get_secrets_policy" {
  ...
  name        = "${var.name_prefix}-cc-${count.index + 1}-get-secrets2-${var.resource_tag}"
...
resource "aws_iam_policy" "cc_session_manager_policy" {
  ...
  name        = "${var.name_prefix}-cc-${count.index + 1}-ssm2-${var.resource_tag}"
...
resource "aws_iam_policy" "cc_autoscale_lifecycle_policy" {
  ...
  name        = "${var.name_prefix}-cc-${count.index + 1}-aslc2-${var.resource_tag}"
...
resource "aws_iam_policy" "cc_metrics_policy" {
  ...
  name        = "${var.name_prefix}-cc-${count.index + 1}-metrics2-${var.resource_tag}"
...
resource "aws_iam_role" "cc_node_iam_role" {
  count              = var.byo_iam == false ? var.iam_count : 0
  name               = var.iam_count > 1 ? "${var.name_prefix}-cc-${count.index + 1}-node-iam-role2-${var.resource_tag}" : "${var.name_prefix}-cc_node_iam_role2-${var.resource_tag}"
...
resource "aws_iam_instance_profile" "cc_host_profile" {
  ...
  name  = var.iam_count > 1 ? "${var.name_prefix}-cc-${count.index + 1}-host-profile2-${var.resource_tag}" : "${var.name_prefix}-cc-host-profile2-${var.resource_tag}"
  role  = aws_iam_role.cc_node_iam_role[count.index].name
