...
resource "aws_iam_role" "ac-iam-role" {
  count = var.ac_count
  name = "${var.name_prefix}-ac-${count.index + 1}-region2-node-iam-role-${var.resource_tag}"
...
resource "aws_iam_instance_profile" "ac-host-profile" {
  count      = var.ac_count
  name       = "${var.name_prefix}-ac-${count.index + 1}-region2-host-profile-${var.resource_tag}"
  role       = aws_iam_role.ac-iam-role.*.name[count.index]
}
