...
resource "aws_iam_role" "workload-region1-iam-role" {
  name = "${var.name_prefix}-workload-region1-iam-role-${var.resource_tag}"
...
resource "aws_iam_role_policy_attachment" "node-AmazonEC2ContainerRegistryReadOnly" {
  ...
  role       = aws_iam_role.workload-region1-iam-role.name
}

resource "aws_security_group" "node-sg-workload" {
  name        = "${var.name_prefix}-workload-region1-sg-${var.resource_tag}"
...
resource "aws_iam_instance_profile" "workload_host_profile" {
  name = "${var.name_prefix}-workload_host_region1_profile-${var.resource_tag}"
  role = aws_iam_role.workload-region1-iam-role.name
}
...
resource "aws_instance" "workload" {
  ...
  tags = merge(var.global_tags,
        { Name = "${var.name_prefix}-workload-region1-${var.resource_tag}" }
  )
}