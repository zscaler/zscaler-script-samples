...
<connection name="Connect to Client Machine (RDP)">
  <protocol>rdp</protocol>
  <param name="hostname">${module.client.public_ip[0]}</param>
  <param name="port">3389</param>
  <!--DEMO CREDENTIALS-->
...
output "client" {
   value = "http://${module.bastion.public_dns}:8080/guacamole/#/client/Q29ubmVjdCB0byBDbGllbnQgTWFjaGluZSAoUkRQKQBjAGRlZmF1bHQ?username=cloudconnector&password=CloudConnector2022!"
}
output "ac1" {
